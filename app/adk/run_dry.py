...
async def _run(scenario: str, mode: str) -> dict:
    director = build_director(autonomous=True)
    sess = session_state.make_session(mode=mode, scenario=scenario)
...
def main():
    parser = argparse.ArgumentParser(description="Arrowow ADK scaffold — DRY_RUN harness")
    ...
    print("=" * 70)
    print(f"ARROWOW ADK SCAFFOLD · mode={args.mode}")
    print(f"scenario: {args.scenario}")
    print("=" * 70)

    summary = asyncio.run(_run(args.scenario, args.mode))
