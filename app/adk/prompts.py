...
def storyboard_instruction(profile: CharacterProfile, no_audio_overlay: bool = False) -> str:
    if no_audio_overlay:
        delivery_style = (
            "DELIVERY — NATURAL CINEMATIC SHORT FILM: this is a professional short-film style video with NO voiceover, "
            "NO spoken dialogue, and NO narrator. The talent does NOT address the camera or talk. Instead, the talent "
            "performs natural, realistic, and professional physical actions in the scenes (e.g., stretching, rolling her shoulders, "
            "moving/interacting with the product naturally). The entire story must be told purely through visual action, "
            "scenery, product interaction, and natural ambient audio (footsteps, room tone, weights). "
            "Ensure transitions flow cleanly and scenes are visually planned to represent a premium commercial short film."
        )
    else:
        delivery_style = (
            "DELIVERY — VOICEOVER B-ROLL: a single consistent voiceover narrates over the action; the "
            "talent performs with her mouth CLOSED and does NOT lip-sync to camera (this avoids lip-sync "
            "failures and keeps one consistent voice across all shots). Brief-specified on-screen text "
            "captions are added in post."
        )
...
def shot_prompt_instruction(profile: CharacterProfile,
                            ...
                            no_audio_overlay: bool = False) -> str:
    if no_audio_overlay:
        delivery_style = (
            "DELIVERY — NATURAL CINEMATIC SHORT FILM: this is a professional short film style video with NO voiceover, "
            "NO spoken dialogue, and NO narrator. Set sync_mode='voiceover' for ALL beats. The talent does NOT talk to the "
        ...
        delivery_style = (
            "DELIVERY — VOICEOVER B-ROLL: she does NOT talk to camera. Her mouth stays gently CLOSED while "
            "she performs the beat's action; a single consistent voiceover (separate TTS track) speaks the "
            "line. Write 'dialogue_or_vo' as the SHORT narration line for this beat (from the brief's "
            "voiceover lines). NEVER pose her mouthing words, talking, or addressing the camera as if "
            "speaking — that breaks lip-sync. She may glance at the lens, but is never mid-speech."
        )
    ...
    return (
        ...
        "lens; lighting; background (the setting from the brief); ambient_audio; sync_mode='voiceover' "
        "for ALL beats (she performs with mouth CLOSED; a separate voiceover narrates — never lip-sync); "
        "seed_locked=true for ALL beats; features_person=true for ALL beats (she is on screen in every "
        "shot); a detailed 'prompt'; 'dialogue_or_vo' (the SHORT voiceover narration line for this beat, "
        "~1 sentence, from the brief's voiceover lines); 'product_action' (how the product is shown/"
        "featured); 'on_screen_text' (the caption from the brief for this beat, e.g. 'Lock-in support' — "
        "it is burned in as a post overlay); AND explicitly generate deeply-analyzed 'start_frame_prompt' "
        "and 'end_frame_prompt' strings.\n"
...
def qa_instruction(profile: CharacterProfile, realism: RealismProfile = UGC_REALISM, no_audio_overlay: bool = False) -> str:
    if no_audio_overlay:
        style_instruction = (
            "NATURAL SHORT FILM STYLE: this is a professional short-film style video using only "
            "the natural ambient audio of the scenes (footsteps, ambient room tone, no voiceover, no "
            "separate narrator, no background music bed). Do NOT penalise the video for missing "
            "voiceover, narration, music, or lip-sync. Score lip_sync_score = 8, audio_score = 8, "
            "and vocal_audio = 8 unless there are severe audio glitches/static. Focus your review "
            "entirely on visual storytelling, camera transitions, visual consistency of the talent "
            "and product, and the overall cinematic flow of the scenes."
        )
    else:
        style_instruction = (
            "VOICEOVER-UGC STYLE: this is a deliberate voiceover ad — a single consistent voice narrates "
            "while the talent performs with her mouth CLOSED (no lip-sync). Do NOT flag lip_sync unless her "
            "mouth clearly attempts synced speech to camera and fails. On-screen text captions and a music "
            "bed are intentional. Watch hard for morphing limbs/fingers and warping/shape-shifting products "
            "during movement (flag 'artifact', high severity) — clean physics is required."
        )
    ...
    return (
        ...
        "plastic/uncanny); lip_sync; audio; continuity; ending_state_score; brief_adherence; product_visibility.\n"
        ...
        "is missing.\n"
        "ENDING STATE (ending_state_score): Is the very last frame of the video sharp, clean, "
        "and unoccluded? If the subject is blurry, walked out of frame, or morphed severely at the end, "
        "score low and add an 'ending_state' defect (severity 4-5) to prevent chaining it to the next shot.\n"
...
def shot_qa_instruction(profile: CharacterProfile, realism: RealismProfile = UGC_REALISM) -> str:
    """Per-SHOT visual QA rubric (one raw 8s clip). Judges VISUALS ONLY — audio, voiceover,
    music and whole-ad brief adherence are added/assessed later at the composite stage, so they
    must NOT be graded here (otherwise every raw clip fails for things it cannot contain)."""
    return (
        "You are a per-SHOT visual QA reviewer for ONE raw 8-second AI clip that will later be cut "
        "into a longer ad. JUDGE VISUALS ONLY.\n"
        "IGNORE ALL AUDIO COMPLETELY — do NOT grade or flag audio, voice, vocal_audio, soundtrack, "
        "music, or lip_sync. The voiceover, music bed and captions are added in post and are NOT in "
        "this clip. Do NOT judge whole-ad brief adherence either — this is a single shot, not the "
        "full story; only assess what THIS shot should show.\n"
        "Score 0-10: overall (visual quality of THIS shot); realism (real phone footage vs AI gloss); "
        "continuity (does the FACE/identity match the locked character AND is the hero product design "
        "consistent); product_visibility (is the hero product shown well, IF this shot features it — "
        "else score 8); ending_state_score (is the LAST frame SHARP, clean, subject in-frame and not "
        "mid-blur/mid-morph, so it can chain into the next shot). Set lip_sync=8 and audio=8 and "
        "brief_adherence=8 (not applicable to a single silent shot).\n"
        "Flag defects ONLY of type: artifact (morphing limbs/fingers, warping), identity_drift (the "
        "FACE changes from the locked character), product (hero product missing/wrong/shape-shifting), "
        "color or hyperrealism (glossy/over-saturated/cinematic grade — must look flat raw smartphone), "
        "framing, ending_state (blurry/occluded/out-of-frame final frame). Each with severity 1-5.\n"
        "SCORING DISCIPLINE: a shot is fundamentally usable unless it has a STRUCTURAL flaw — a wrong/"
        "changed FACE, morphing limbs or a shape-shifting product, or a blurry/occluded final frame. Do "
        "NOT let mild Veo polish/smoothness, slightly strong colour, or ordinary framing alone drop "
        "'overall' below 6; reserve low overall (<6) and severity>=4 for genuine structural defects. "
        "APPROVE when the face matches the character, there is no morphing, and the final frame is usable. "
        f"The talent's FIXED identity: {profile.identity_lock()[:200]}. "
        f"Footage must look real, not AI: {realism.negative_block()}"
    )
