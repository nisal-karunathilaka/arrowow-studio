...
def shot_prompt_instruction(profile: CharacterProfile,
    ...
    return (
        ...
        f"CHARACTER LOCK — describe the subject in EVERY prompt using EXACTLY: "
...
def qa_instruction(profile: CharacterProfile, realism: RealismProfile = UGC_REALISM) -> str:
    return (
        ...
        "consistent). List concrete defects, each with: type (one of lip_sync, vocal_audio, "
...
def build_beat_generation_prompt(beat: dict, profile: CharacterProfile,
    ...
    if native:
        ...
        perform = ("She does NOT look at or speak to the camera — her voice plays only as "
                   "voiceover while she moves; no lip-sync required. ")
        line = f'(Voiceover, Sienna: "{beat.get("dialogue_or_vo","")}")'
    ...
    positive = (
        ...
        f"{realism.directive_block()}{boost}\n"
